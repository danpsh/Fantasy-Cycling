...
def render_dashboard(league_key, title):
    ...
    for i, name in enumerate(order):
        score = int(data["lb"][data["lb"]['owner'] == name]['pts'].sum())
...
def show_point_history():
    ...
    if not proc.empty:
        ...
        ytd_disp = ytd[['Date_Str', 'Race Name', 'Full_Stage', 'rider_name', 'owner', 'Place_Label', 'pts']]
        ytd_disp.columns = ['Date', 'Race', 'Stage', 'Rider', 'Owner', 'Place', 'Points']
        ...
        with st.expander(f"**{label}** — Tanner: {t_pts} | Daniel: {d_pts}"):
            ...
            for i, owner in enumerate(["Tanner", "Daniel"]):
                with (c1 if i==0 else c2):
                    df = pts[(pts['owner'] == owner) & (pts['team_pick'] >= start) & (pts['team_pick'] <= end)]
...
def show_schedule():
    ...
    disp = schedule_df[['date', 'race_name', 'tier', 'race_type']].copy()
    disp.columns = ['Date', 'Race', 'Tier', 'Race Type']
