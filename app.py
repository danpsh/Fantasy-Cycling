...
if results_raw is not None and riders_df is not None and schedule_df is not None:
    # Maintain CSV draft order
    riders_df['draft_pos'] = range(len(riders_df))
    ...
    df_long = results_raw.melt(id_vars=['Date', 'Race Name', 'Stage', 'Race_ID'], 
                               value_vars=rank_cols, var_name='Pos_Label', value_name='rider_name')
    ...
    # --- 4. SINGLE UNIFIED ROSTER TABLE ---
    st.subheader("Master League Roster")
    
    # Build the unified table
    master_roster = riders_df.merge(rider_points, left_on='rider_name', right_on='rider_name_y', how='left').fillna(0)
    master_roster = master_roster[['rider_name', 'owner', 'pts']].copy()
    master_roster.columns = ['Rider', 'Owner', 'Total Points']
    
    # Function to color rows based on owner
    def color_rows(row):
        color = '#1f77b422' if row.Owner == 'Daniel' else '#ff7f0e22'
        return [f'background-color: {color}'] * len(row)

    st.dataframe(master_roster.style.apply(color_rows, axis=1), 
                 hide_index=True, 
